...
function getReviews(productId) {
  if (!reviewsCache[productId]) {
    ...
    const average =
      Math.round((4.3 + Math.random() * 0.6) * 10) / 10;
...
function renderProducts() {
  ...
  productsGrid.innerHTML = filtered.map(product => {
    const review = getReviews(product.id);

    const unavailable = Number(product.price) === 0;
      ...
      <article class="product-card" data-product-id="${product.id}">
        ...
        <div class="product-info">
          ...
          <div class="product-rating">
            <span class="stars">${stars(review.average)}</span>
  ...
  productsGrid
    ...
    .forEach(button => {
      button.addEventListener("click", () => {
        const product = products.find(
          item => item.id === button.dataset.reviews
        );
        ...
        openModal(
          ...
          `
            <div class="reviews-modal">
              ...
              <p>
                Basé sur ${review.count} avis clients.
              </p>
...
/*
  ...
  Cette fonction nettoie complètement les anciennes données
  du localStorage.

  Si un ancien panier contient :
  - undefined
  - NaN
  - -1
  - une image placeholder
  - un ancien produit supprimé

  l'article est supprimé automatiquement.
*/
...
function loadCart() {
  ...
  /*
    On nettoie immédiatement l'ancien panier.
    Le mauvais article ne pourra donc plus revenir.
  */
  sanitizeCart();
  saveCart();
  renderCart();
...
function updateCartBadge() {
  ...
  cartBadge.textContent = quantity > 99
    ? "99+"
    : String(quantity);
...
function flyProductToCart(productCard, button) {
  const image =
    productCard?.querySelector(".product-image");

  const cartTarget = cartBtn;
  ...
  clone.style.width = `${Math.min(imageRect.width, 180)}px`;
  clone.style.height = `${Math.min(imageRect.height, 180)}px`;
  ...
  const endX =
    cartRect.left +
    cartRect.width / 2 -
    Math.min(imageRect.width, 180) / 2;

  const endY =
    cartRect.top +
    cartRect.height / 2 -
    Math.min(imageRect.height, 180) / 2;
...
function addToCart(product, event, button) {
  ...
  if (!Number.isFinite(price) || price <= 0) {
    showToast("Produit indisponible pour le moment.", "error");
    return;
  }
  ...
  flyProductToCart(productCard, button);
...
function renderCart() {
  ...
  /*
    Nettoyage avant CHAQUE rendu.
    Impossible d'afficher undefined / NaN / quantité négative.
  */
  sanitizeCart();
  saveCart();
  ...
  cartItems.innerHTML = cart.map(item => {
    ...
    /*
      On récupère TOUJOURS le produit original.
      Donc pas d'ancien nom/image/prix corrompu.
    */
    const product = products.find(
      product => product.id === item.id
    );
  ...
  cartItems
    .querySelectorAll(".cart-item-image")
    .forEach(setupImageFallback);
  ...
  openModal(
    ...
    `
      <form id="authForm" class="auth-form">
        ...
        <button type="submit" class="modal-action">
    ...
    if (!orders.length) {
      container.innerHTML = `
        <div class="empty-orders">
          ...
          <p>Tu n'as encore aucune commande.</p>
        </div>
      `;
...
checkoutBtn?.addEventListener(
  ...
  async () => {
    ...
    try {
      ...
      checkoutBtn.disabled = true;
      checkoutBtn.textContent = "Commande...";
